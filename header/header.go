...
func (r *RequestHeader) Marshal() []byte {
	...
	// note: slice is a reference type, means the modify on writeString func will change the var header
...
func (r *RequestHeader) Unmarshal(data []byte) (err error) {
	...
	// `defer` ensures when panic happens, run the below func immediately
	defer func() {
		if r := recover(); r != nil {
			err = UnmarshalError
		}
...
// read str from data, with return value str and its length
func readString(data []byte) (string, int) {
	idx := 0
	length, size := binary.Uvarint(data) // length is the decoded uint64 number, size is the len of decoded data
	idx += size
...
// turn str into varint and write into data
func writeString(data []byte, str string) int {
	idx := 0
	idx += binary.PutUvarint(data, uint64(len(str))) // PutUvarint turn a uint64 into a varint buf and write into slice, with return value is the number of written bytes
	copy(data[idx:], str)
	idx += len(str)
	return idx
}
